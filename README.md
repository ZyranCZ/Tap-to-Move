...
Tap to Move includes native compatibility with both the **Dramatic Shape Voxel Mod** and **PotatoVoxel**.

Instead of pretending the 3D scene is still a flat 2D grid, the mod reads the active renderer's exported Voxel camera and scene information through one provider adapter. Touches are resolved through the actual 3D view so buildings, walls, raised surfaces, NPCs and ground are interpreted from the same scene the player sees.
...
Dramatic Shape and PotatoVoxel are **optional dependencies**. If neither is installed, or Voxel mode is disabled, Tap to Move automatically uses its normal 2D targeting path. PotatoVoxel uses the full normal 3D navigation system and does **not** require Simple Mode.
...
Voxel integrations use each renderer's exported companion-library interface rather than modifying renderer internals. Dramatic Shape and PotatoVoxel share the same Tap to Move navigation implementation through the provider adapter.
